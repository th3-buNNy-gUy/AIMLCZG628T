...
def test_model(model, scaler, dataframe, anomaly_column, description_column):
...
def logistic_regression(dataframe, anomaly_column, description_column, solver, C):
...
def support_vector_machine_classifier(dataframe, anomaly_column, description_column, kernel, C):
    X_train = dataframe.drop(columns=[anomaly_column, description_column]).values
    y_train = dataframe[anomaly_column].values.ravel()
    ...
    model = SVC(kernel=kernel, C=C) # Example with RBF kernel
    model.fit(X_train_scaled, y_train)
    return model, scaler

def extreme_gradient_boosting_classifier(dataframe, anomaly_column, description_column, lr, n_estimators):
    X_train = dataframe.drop(columns=[anomaly_column, description_column]).values
    y_train = dataframe[anomaly_column].values.ravel()
...
def random_forest_classifier(dataframe, anomaly_column, description_column, criterion, n_estimators):
    X_train = dataframe.drop(columns=[anomaly_column, description_column]).values
    y_train = dataframe[anomaly_column].values.ravel()
